...
LRESULT CALLBACK GlassWindow::WndProc(HWND _hwnd, UINT _message, WPARAM _wParam, LPARAM _lParam)
{
	...
	switch (_message)
	{
		...
		case WM_RBUTTONDOWN: {
			GW->StopMove();
			//RECT rc;
			//GetWindowRect(GetDesktopWindow(), &rc);			//获取桌面大小
			//LPoint PetSize = GW->GetWindowSize();				//获取窗口大小
			//LPoint pt;
...
void GlassWindow::initPet()
{
	...
	string SpriteName(VcString[0]);			//精灵名称		如："Cube"
	string Suffix(VcString[1]);				//精灵纹理名称	如：".png"
	vector<float> VcSpriteSize;
	Lin::ReadFileData_To_FloatVector(MainFile, VcSpriteSize, ValidDataSpriteSize);
	LPoint SpriteSize{ VcSpriteSize[0], VcSpriteSize[1] };	//精灵尺寸
